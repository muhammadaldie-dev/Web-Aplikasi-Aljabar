...
function inverseMatrixWithSteps(matrix) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (Math.abs(aug[i][i]) < 1e-10) {
            ...
            for (let j = i + 1; j < n; j++) {
                if (Math.abs(aug[j][i]) > 1e-10) {
                    ...
                    stepDiv.textContent = `Tukar R${i+1} dengan R${j+1}`;
        ...
        stepDiv1.textContent = `Bagi R${i+1} dengan ${formatNumber(pivot)}`;
        ...
        for (let j = 0; j < n; j++) {
            if (j !== i) {
                ...
                if (Math.abs(factor) > 1e-10) {
                    ...
                    stepDiv2.textContent = `R${j+1} = R${j+1} - (${formatNumber(factor)})R${i+1}`;
